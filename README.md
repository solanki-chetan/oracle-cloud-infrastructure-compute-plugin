...
3. Click **Advanced** for more options:
   ...
   - **JenkinsAgentUser** -  The custom user to start the Jenkins agent process. This user must be baked into the OS image you select or created through an init script. If the specified user matches the SSH user, no sudo privileges are required. If a different user is specified, the SSH user must have sudo privileges to switch to that user.
